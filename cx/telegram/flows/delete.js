...
async function handleDelete(chatId, messageText, userStates, API_URL) {
  ...
  if (messageText === "/delete") {
    ...
    try {
      ...
      await axios.put(`${config.backendURL}/api/delete-user/${chatId}`);

      delete userStates[chatId];
      await axios.post(`${API_URL}/sendMessage`, {
        chat_id: chatId,
        text:
          "🎉 <b>You have deleted your entry from the database. Please do /start " +
          "to re-register now. </b> 😊",
        parse_mode: "HTML", // Enables bold and clean formatting
      });
    } catch (error) {
      console.error(
        "Error trying to delete entry from database:",
        error.message
      );
    }
  }
